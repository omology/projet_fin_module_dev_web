...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <x-home.bookSection/>
    {{-- <x-home.bookSectionCaroussel :books="$mostLikedBooks"/> --}}
...
$(document).ready(function() {
    // Example AJAX request (you can replace this with your actual form submission or action)
    $('#sendMessageForm').on('submit', function(e) {
        e.preventDefault(); // Prevent form from submitting normally
        
        $.ajax({
            url: "{{ route('home.message.store') }}", // Replace with your route
            type: "POST",
            data: $(this).serialize(), // Serialize form data
            success: function(response) {
                // On success, show the success alert with animation
                $('#alertMessage').text(response.message); // Set dynamic message
                $('#successAlert')
                    .removeClass('hidden') // Show the alert
                    .removeClass('translate-y-[-100px]') // Remove translation effect (from top)
                    .addClass('translate-y-0') // Position it normally (translate to 0)
                    .fadeIn(500); // Fade-in effect for smooth appearance

                // Hide the alert after 3 seconds
                setTimeout(function() {
                    $('#successAlert')
                        .fadeOut(500) // Fade-out effect for smooth disappearance
                        .addClass('hidden'); // Keep it hidden for next use
                }, 5000);
                $('#sendMessageForm')[0].reset();
            },
            error: function(xhr, status, error) {
                // On error, show the error alert
                var errorMessage = xhr.responseJSON && xhr.responseJSON.message
                    ? xhr.responseJSON.message
                    : "Something went wrong, please try again.";

                $('#errorMessage').text(errorMessage); // Set dynamic error message
                $('#errorAlert')
                    .removeClass('hidden') // Show the error alert
                    .removeClass('translate-y-[-100px]') // Remove translation effect (from top)
                    .addClass('translate-y-0') // Position it normally (translate to 0)
                    .fadeIn(300); // Fade-in effect for smooth appearance
